...
async def list_network_devices(tid: str, user=Depends(get_current_user)):
    ...
    rows = await fetch(
        """SELECT id, name, description, ip_address, device_type, vendor, model, location,
    ...
    try:
        row = await fetchrow(
            """INSERT INTO network_devices
                 (tenant_id, name, description, ip_address, device_type, vendor, model, location,
            ...
            tid, body.name, _n(body.description), body.ipAddress, body.deviceType, _n(body.vendor), _n(body.model), _n(body.location),
...
async def update_network_device(tid: str, did: str, req: Request, body: NetworkDeviceUp, user=Depends(get_current_user)):
    ...
    row = await fetchrow(
        """UPDATE network_devices SET
             name=$1, description=$2, ip_address=$3, device_type=$4, vendor=$5, model=$6, location=$7,
             snmp_port=$8, snmp_version=$9,
             community_enc = COALESCE($10, community_enc),
             v3_username=$11, v3_security_level=$12, v3_auth_protocol=$13,
             v3_auth_password_enc = COALESCE($14, v3_auth_password_enc),
             v3_priv_protocol=$15,
             v3_priv_password_enc = COALESCE($16, v3_priv_password_enc),
             poll_interval_sec=$17, raw_retention_hours=$18, rollup_bucket_minutes=$19,
             rollup_retention_days=$20
           WHERE id=$21 AND tenant_id=$22
           RETURNING id, name, ip_address, device_type, snmp_version, status""",
        body.name, _n(body.description), body.ipAddress, body.deviceType, _n(body.vendor), _n(body.model), _n(body.location),
        body.snmpPort, body.snmpVersion,
        encrypt(body.community) if body.community else None,
        _n(body.v3Username), _n(body.v3SecurityLevel), _n(body.v3AuthProtocol),
        encrypt(body.v3AuthPassword) if body.v3AuthPassword else None,
